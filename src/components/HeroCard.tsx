...
const cardContents = [
  {
    id: 1,
    layout: 'split',
    user: { name: 'Ahmad Kurniawan', role: 'Software Engineer', avatar: 'AK' },
  ...
  },
  {
    id: 2,
    layout: 'stacked',
    user: { name: 'Siti Rahayu', role: 'Data Analyst', avatar: 'SR' },
  ...
  },
  {
    id: 3,
    layout: 'grid',
    user: { name: 'Budi Santoso', role: 'Financial Analyst', avatar: 'BS' },
  ...
  },
  {
    id: 4,
    layout: 'minimal',
    user: { name: 'Dewi Lestari', role: 'Mathematics Teacher', avatar: 'DL' },
  ...
  },
  {
    id: 5,
    layout: 'card-list',
    user: { name: 'Rizki Pratama', role: 'Marketing Strategist', avatar: 'RP' },
...
function Avatar({ initials }: { initials: string }) {
  return (
    <div className="flex size-8 shrink-0 items-center justify-center border border-foreground/20 bg-foreground/5 text-xs font-medium tracking-wide text-foreground">
      {initials}
    </div>
  );
}
...
function SplitLayout({ card }: { card: (typeof cardContents)[0] }) {
  return (
    <div className="grid h-full grid-cols-[1fr_180px]">
      <div className="flex flex-col gap-6 border-r border-foreground/8 p-6">
      ...
      <div className="grid grid-cols-2 gap-2">
        {card.metrics?.map((m, i) => (
          <motion.div
            ...
            className="border border-foreground/8 p-5"
          >
            <p className="text-4xl font-semibold tabular-nums">{m.value}</p>
        ...
        {(card as any).insights?.map(
          (ins: { label: string; value: string }, i: number) => (
            <motion.div
              ...
              className="flex items-center justify-between border-t border-foreground/8 py-3 text-xs"
            >
      ...
      <div className="grid flex-1 grid-cols-2 gap-2">
        {card.topics?.map((t, i) => (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            transition={{ delay: i * 0.08 }}
            className="flex flex-col justify-between border border-foreground/8 p-5"
          >
            <div className="flex items-center justify-between">
              <span
            ...
            <p className="mt-3 text-xs text-foreground/50">{t.name}</p>
          </motion.div>
        ))}
      </div>
      <HashLine hash={card.hash} />
  ...
  return (
    <div className="flex h-full flex-col gap-6 p-6">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <Avatar initials={card.user.avatar} />
          <div>
      ...
      <div className="flex flex-1 flex-col gap-2">
        {card.achievements?.map((a, i) => (
          <motion.div
            ...
            transition={{ delay: i * 0.1 }}
            className="flex items-center justify-between border-t border-foreground/8 py-4 text-xs"
          >
            <div className="flex items-center gap-3">
              <span className="text-[10px] uppercase tracking-widest text-foreground/30">
                0{i + 1}
...
export default function HeroCard() {
  ...
  return (
    <div className="relative flex min-h-105 items-center justify-center">
      <div className="absolute -right-15 top-1/2 w-[120%] -translate-y-1/2 overflow-hidden border border-foreground/10 bg-background">
