...
function AnimatedChartColumn({
  ...
  return (
    ...
    >
      ...
      <div
        data-slot="animated-charts-column-title-wrapper"
        className="absolute top-0 left-0 right-0 p-2 px-3"
      >
        <span
          data-slot="animated-charts-column-title"
          className={cn(
            'text-base font-normal text-foreground/50',
            globalTitleClassName,
            columnTitleClassName,
          )}
        >
      ...
      <div className="relative flex-1 flex flex-col justify-end border-t border-border/10">
        ...
        <motion.div
          ...
          className={cn(
            'relative w-full border-t-2 border-border/30 bg-muted/20',
            columnClassName,
            topBorderClassName,
          )}
          initial={{ height: 0 }}
...
function AnimatedChart({
  ...
  const dataSignature = useMemo(() => {
    return JSON.stringify(
      columns.map((c) => ({ title: c.title, value: c.value })),
    );
  }, [columns]);
  ...
  return (
    <div
      ...
      className={cn('flex w-full h-full gap-0 border', className)}
      {...props}
    >
