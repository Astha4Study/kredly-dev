...
export function OrbitingCirclesAnimation() {
  return (
    <div className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden">
      {/* Center Label */}
      <div className="absolute z-10 flex h-14 w-14 items-center justify-center rounded-full border border-zinc-200 bg-zinc-100">
        <span className="text-xs font-bold">Kredly</span>
      </div>

      {/* Orbit 1 - Inner with icons */}
      <OrbitingCircles iconSize={29} radius={60}>
        <Icons.infojobs />
        <Icons.linkedin />
        <SiIndeed />
        <SiGlassdoor />
      ...
      {/* Orbit 3 - Outer with icons */}
      <OrbitingCircles iconSize={26} radius={120} reverse speed={2}>
        <Icons.infojobs />
        <FaSquareUpwork />
        <FaGithub />
        <FaBehance />
