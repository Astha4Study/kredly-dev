import { createFileRoute } from '@tanstack/react-router';
import { Button } from '@/components/ui/button';
import { Download, Loader2 } from 'lucide-react';
import { useState, useMemo, useEffect } from 'react';
import StatisticsCertificationCard from '@/pages/dashboard/certification/StatisticsCertificationCard';
import SearchAndFiltersCertification from '@/pages/dashboard/certification/SearchAndFiltersCertification';
import CertificationList from '@/pages/dashboard/certification/CertificationList';
import { useAuth } from '@/contexts/auth';
import { sessionService } from '@/services/sessionService';
import ResultCertModal from '@/pages/client/cat/result/ResultCertModal';
import { Card } from '@/components/ui/card';

export const Route = createFileRoute('/_app/app/certification/')({
  component: RouteComponent,
});

interface Credential {
  id: string;
  skillName: string;
  score: number;
  dateEarned: string;
  blockchainTxHash: string;
  status: 'verified';
  sessionId?: string;
  level?: string;
}

function RouteComponent() {
  const { user } = useAuth();
  const [sortBy, setSortBy] = useState('date');
  const [searchQuery, setSearchQuery] = useState('');
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [filterStatus, setFilterStatus] = useState<'all' | 'verified'>('all');

  const [credentials, setCredentials] = useState<Credential[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  // States for certificate detail modal
  const [selectedCredential, setSelectedCredential] =
    useState<Credential | null>(null);
  const [detailedResult, setDetailedResult] = useState<any | null>(null);
  const [isLoadingResult, setIsLoadingResult] = useState(false);
  const [showCertModal, setShowCertModal] = useState(false);

  // Fetch completed assessments on mount
  useEffect(() => {
    async function fetchCompletedAssessments() {
      try {
        const response = await fetch('/api/profile', {
          credentials: 'include',
        });
        if (response.ok) {
          const data = await response.json();
          if (data.profile && data.profile.cvAssessments) {
            const completed = data.profile.cvAssessments.filter(
              (a: any) => a.status === 'completed',
            );
            const mapped = completed.map((a: any) => {
              const hash = a.sessionId
                ? `0x${a.sessionId.replace(/-/g, '').slice(0, 40)}`
                : '0x0000000000000000000000000000000000000000';

              return {
                id: a.id,
                skillName: a.title,
                score: a.score || 0,
                dateEarned: 'Selesai',
                blockchainTxHash: hash,
                status: 'verified' as const,
                sessionId: a.sessionId,
                level: a.level || 'Intermediate',
              };
            });
            setCredentials(mapped);
          }
        }
      } catch (err) {
        console.error('Failed to fetch completed assessments:', err);
      } finally {
        setIsLoading(false);
      }
    }
    fetchCompletedAssessments();
  }, []);

  const handleSelectCredential = async (cred: Credential | null) => {
    if (!cred) {
      setSelectedCredential(null);
      setDetailedResult(null);
      setShowCertModal(false);
      return;
    }

    setSelectedCredential(cred);

    if (cred.sessionId) {
      setIsLoadingResult(true);
      try {
        const res = await sessionService.getResult(cred.sessionId);
        setDetailedResult(res);
        setShowCertModal(true);
      } catch (err) {
        console.error('Failed to fetch result detail:', err);
      } finally {
        setIsLoadingResult(false);
      }
    } else {
      setDetailedResult({
        candidate_name: user?.name || 'Pengguna Kredly',
        role: cred.skillName,
        level: cred.level || 'Intermediate',
        score: cred.score,
        verification_id: 'MOCK-CERT-' + cred.id,
        total_items: 20,
        duration_seconds: 1800,
      });
      setShowCertModal(true);
    }
  };

  // Filter and sort credentials
  const filteredCredentials = useMemo(() => {
    let filtered = [...credentials];

    // Search filter
    if (searchQuery) {
      filtered = filtered.filter((cred) =>
        cred.skillName.toLowerCase().includes(searchQuery.toLowerCase()),
      );
    }

    // Status filter
    if (filterStatus !== 'all') {
      filtered = filtered.filter((cred) => cred.status === filterStatus);
    }

    // Sort
    filtered.sort((a, b) => {
      if (sortBy === 'score') return b.score - a.score;
      if (sortBy === 'skill') return a.skillName.localeCompare(b.skillName);
      return 0; // date default
    });

    return filtered;
  }, [credentials, searchQuery, filterStatus, sortBy]);

  // Calculate statistics
  const stats = useMemo(() => {
    const totalCredentials = credentials.length;
    const averageScore =
      credentials.reduce((acc, curr) => acc + curr.score, 0) /
      (totalCredentials || 1);
    const latestDate = credentials[0]?.dateEarned || '-';

    return {
      total: totalCredentials,
      average: Math.round(averageScore),
      latest: latestDate,
    };
  }, [credentials]);

  if (isLoading) {
    return (
      <div className="flex h-[50vh] items-center justify-center">
        <Loader2 className="h-8 w-8 text-primary animate-spin" />
      </div>
    );
  }

  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="space-y-6">
        {/* Header Section */}
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold tracking-tight">
              Kredensial Saya
            </h2>

            <p className="mt-1 text-sm text-muted-foreground">
              Lihat dan kelola seluruh kredensial yang telah Anda peroleh.
            </p>
          </div>

          <Button variant="outline" className="gap-2">
            <Download className="h-4 w-4" />
            Export All
          </Button>
        </div>

        {/* Statistics Cards */}
        <StatisticsCertificationCard
          total={stats.total}
          average={stats.average}
          latest={stats.latest}
        />

        {/* Search and Filters */}
        <SearchAndFiltersCertification
          searchQuery={searchQuery}
          setSearchQuery={setSearchQuery}
          filterStatus={filterStatus}
          setFilterStatus={setFilterStatus}
          sortBy={sortBy}
          setSortBy={setSortBy}
          viewMode={viewMode}
          setViewMode={setViewMode}
        />

        {/* Certification Grid/List */}
        <CertificationList
          filteredCredentials={filteredCredentials}
          viewMode={viewMode}
          setSelectedCredential={handleSelectCredential}
          searchQuery={searchQuery}
          filterStatus={filterStatus}
          setSearchQuery={setSearchQuery}
          setFilterStatus={setFilterStatus}
        />

        {/* Loading overlay for fetching result */}
        {isLoadingResult && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center z-50 p-4">
            <Card className="max-w-xs w-full p-6 text-center space-y-4">
              <Loader2 className="h-10 w-10 text-primary animate-spin mx-auto" />
              <p className="text-sm font-medium text-muted-foreground">
                Mengambil data sertifikat...
              </p>
            </Card>
          </div>
        )}

        {/* ResultCertModal rendering */}
        {detailedResult && (
          <ResultCertModal
            show={showCertModal}
            onClose={() => {
              setShowCertModal(false);
              setSelectedCredential(null);
              setDetailedResult(null);
            }}
            userName={
              detailedResult.candidate_name || user?.name || 'Pengguna Kredly'
            }
            role={detailedResult.role || selectedCredential?.skillName || ''}
            level={
              detailedResult.level ||
              selectedCredential?.level ||
              'Intermediate'
            }
            score={detailedResult.score || selectedCredential?.score || 0}
            verificationId={detailedResult.verification_id || ''}
            totalItems={detailedResult.total_items || 20}
            durationSeconds={detailedResult.duration_seconds || 1800}
            sessionId={selectedCredential?.sessionId}
          />
        )}
      </div>
    </main>
  );
}
