...
- **`store.SessionStore`**: Penyimpanan sesi ujian dalam memori (*in-memory*) yang dilengkapi mutex untuk keamanan konkurensi (*thread-safe*).
...
*   **Refill Queue**: Setiap kali soal diambil dari antrean, jika antrean sisa `< 3`, sistem memicu background prefetch kembali secara *asynchronous* untuk mengisi ulang antrean.
*   **Masking Informasi Sensitif**: Di tingkat HTTP Handler (`HandleNextItem`), properti `KunciJawaban`, `Penjelasan`, dan tingkat kesulitan soal asli (`BEstimated`) disembunyikan menggunakan struct `FilteredItem` agar kandidat tidak bisa melakukan *cheat* melalui inspeksi network browser.

---

### 3. Pengiriman Jawaban (`SubmitAnswer`)
Kandidat mengirimkan pilihan jawaban mereka ('A', 'B', 'C', atau 'D').
*   **Koreksi**: Sistem membandingkan jawaban dengan kunci secara *case-insensitive*.
...
*   Nilai akhir $\theta$ selalu di-*clamp* dalam rentang $[-4.0, +4.0]$.
...
| **GET** | `/api/sessions/:id/next-item` | Mengambil soal berikutnya | *None* | `{"item": {"id": "...", "topic": "...", "pertanyaan": "...", "pilihan": [...]}, "question_number": 1}` |
...
| **GET** | `/api/sessions/:id/result` | Mengambil hasil akhir & evaluasi AI | *None* | `{"score": 560, "theta": 0.5, "level": "Intermediate", "feedback": "...", "strengths": [...], "weaknesses": [...], "recommendations": [...], "verification_id": "CERT-..."}` |
