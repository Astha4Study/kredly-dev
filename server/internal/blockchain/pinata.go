...
func (p *PinataClient) UploadImage(imageBuffer []byte, filename string, mimeType string) (string, error) {
	if p == nil {
		return "", fmt.Errorf("Pinata client not initialized - check PINATA_JWT")
	}
	...
	if _, err := part.Write(imageBuffer); err != nil {
		return "", fmt.Errorf("failed to write image buffer: %w", err)
	}
...
// DeleteFile deletes a file from Pinata by CID
func (p *PinataClient) DeleteFile(cid string) error {
	...
	resp, err := client.Do(req)
	if err != nil {
		return fmt.Errorf("failed to delete from Pinata: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		bodyBytes, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("Pinata delete failed with status %d: %s", resp.StatusCode, string(bodyBytes))
	}
...
// ExtractCIDFromURL extracts CID from IPFS gateway URL
func ExtractCIDFromURL(url string) string {
	// Extract CID from URLs like:
	// https://gateway.pinata.cloud/ipfs/QmXXXX
	// https://ipfs.io/ipfs/QmXXXX
	if url == "" {
		return ""
	}
	parts := bytes.Split([]byte(url), []byte("/ipfs/"))
	if len(parts) < 2 {
		return ""
	}
